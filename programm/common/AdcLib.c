...
/**
	...
	@param pPort :{pADI_ADC0, pADI_ADC1}
		- pADI_ADC0 for ADC0.
		- pADI_ADC1 for ADC1.
	@param iRef :{ADCCON_ADCREF_INTREF, ADCCON_ADCREF_EXTREF, ADCCON_ADCREF_EXTREF2, ADCCON_ADCREF_AVDDREFD}
		- ADCxCON.12,13
...
/**
	...
	@param pPort :{pADI_ADC0, pADI_ADC1}
		- pADI_ADC0 for ADC0.
		- pADI_ADC1 for ADC1.
	@param iSF :{ADCFLT_SF_MSK}
		- Set iSF to desired Sinc Decimation factor:
	@param iAF :{ADCFLT_AF_MSK}	ADCxFLT.8-11.
	...
	@return ADCxFLT MMR.  Not normally of interest.
		Consult data sheet if required.
**/
int AdcFlt(ADI_ADC_TypeDef *pPort, int iSF, int iAF, int iFltCfg)
	{
	int	i1 = 0;
	//i1 = (iSF&0x7f)|(iAF&0xf00)|(iFltCfg&0xD080);	
	i1 = (iSF&0x7f)|((iAF<<8)&0xf00)|(iFltCfg&0xD080);// ИСПРАВИЛ
	pPort->FLT = i1;
	return pPort->FLT;
	}
...
/**
	...
	@param iBiasBoost :{ADC_BIAS_X1, ADCCFG_BOOST15, ADCXCFG_BOOST30}
	- ADC0CFG.12
		- 0 or ADC_BIAS_X1 for normal Vbias.
		- 4096 or ADCCFG_BOOST15 to boost Vbias by 15 times.
...
/**
	...
	@param pPort :{pADI_ADCSTEP,}	
			- pADI_ADCSTEP.
	@param iCtrl :{0|DETCON_SINC2_EN|DETCON_STEPCTRL|DETCON_REFDET}
		- 0 or SINC2 and Reference detection off
		- 0x8 or DETCON_STEPCTRL Step detection compared with main filter output
