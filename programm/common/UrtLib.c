...
/**
	...
	@param iBrk :{COMLCR_BRK_CLR, COMLCR_BRK_SET}	\n
		- 0 = COMLCR_BRK_CLR to disable SOUT break condition (SOUT behaves as normal)
		- 1 = COMLCR_BRK_SET to force SOUT break condition - SOUT remains low until this bit is cleared
	@return Value of COMLSR: See UrtLinSta() function for bit details.
...
int UrtTx_f(ADI_UART_TypeDef *pPort, int iTx)
	{
	pPort->COMTX = iTx;
	return 0;
	}
